  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-neutral-500">
        Daily signal digests — grouped by date, newest first.
      </p>

      {dates.length === 0 ? (
        <div className="mt-6 rounded-md border p-4 text-sm text-neutral-500">
          No digests yet — check back after the next pipeline run.
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-neutral-500">
                      {dayEvents.length} signal{dayEvents.length !== 1 ? 's' : ''} —{' '}
                      {companies.join(', ')}{companies.length < [...new Set(dayEvents.map((e) => e.company))].length ? '…' : ''}
                    </div>
                  </div>
                ...
                <div className="mt-4 space-y-2">
                    ...
                    <div key={event.id ?? idx} className="flex items-start justify-between gap-4 rounded-md border p-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2 text-xs text-neutral-500 mb-1">
                          ...
                          {event.primary_tag && (
                            <span className="rounded-full bg-neutral-100 px-2 py-0.5">{event.primary_tag}</span>
                          )}
                          {event.impact_score && (
                        ...
                        {event.summary && (
                          <div className="mt-1 text-xs text-neutral-500 line-clamp-2">{event.summary}</div>
                        )}
                      </div>
                      {event.link && (
                  ...
                  {dayEvents.length > 5 && (
                    <div className="text-xs text-neutral-400 pt-1">
                      +{dayEvents.length - 5} more signals this day
                    </div>
                  )}
                </div>
              </div>
