...
const features = [
  { title: 'Multi-source coverage', copy: 'AI labs, fintech product updates, regulator dockets, and SEC filings combine into one stream.' },
  { title: 'Push delivery', copy: 'Telegram, email, and webhook automation keep the team in sync without dashboard refreshes.' },
  { title: 'Operator + trader modes', copy: 'Filters for ops, risk, and trading desks so everyone sees signals mapped to their playbook.' },
  { title: 'Built-in API + webhooks', copy: 'Stripe checkout provisions real API keys instantly. Pipe data into your models or alerting stack.' },
];

const personas = [
  { title: 'Fintech operators', copy: 'Catch competitor launches, pricing moves, and regulatory heat before the press release drops.' },
  { title: 'Traders & analysts', copy: 'Flash SEC alerts plus context from AI + fintech sources to size impact within minutes.' },
];
...
const tiers = [
  ...
  },
  {
    ...
    features: ['Everything in Free', 'Flash SEC alerts — 8-K, 13D/G, Form 4', '28+ live sources', 'Telegram push alerts with tag filtering', 'Market pair impact analysis', 'Self-serve API key', 'Digest archive + social drip queue', '6-hourly pipeline, 24/7'],
    locked: ['AI trade direction predictions', 'Daily AI briefing', 'Email digest delivery'],
  ...
  },
  {
    ...
    features: ['Everything in Pro', 'AI trade predictions with confidence score', 'Signals 30 min before Pro tier', 'Daily AI briefing at 7am', 'Pre-event alerts 15 min before red folder events', 'Post-event AI debrief + updated trade setups', 'Weekly macro outlook every Sunday', 'AI Trading Assistant', 'Elite badge in Traders Circle', 'Unlimited API calls'],
    locked: [],
...
const reasons = [
  { icon: '⚡', title: 'Speed over opinion', copy: 'We push signals within minutes of a filing dropping. Trading mentors post hours later — after the move already happened.' },
  { icon: '📡', title: '28+ verified sources', copy: 'Not one person\'s calls. SEC EDGAR, central banks, AI labs, fintech companies, crypto news, forex events — all automated.' },
  { icon: '🤖', title: 'AI analysis, not vibes', copy: 'Every signal is analysed by Claude AI — market pair impact, trade direction, entry zone, target and stop loss. Data-driven, not gut feeling.' },
  { icon: '🔔', title: 'Delivered to you', copy: 'Signals come to your Telegram automatically. You don\'t check a dashboard — we push the moment it matters.' },
  { icon: '🔑', title: 'API access included', copy: 'Build your own tools on top of our data. Pull signals into Zapier, n8n, or your own trading stack.' },
  { icon: '💷', title: 'Fraction of the cost', copy: 'Trading courses charge £1,000+. Private groups charge £3,000+. We charge £45/month for automated institutional-grade intel.' },
];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setActive((prev) => Math.max(0, prev - 1))} disabled={active === 0} className="h-8 w-8 rounded-full border border-white/20 text-white/60 hover:bg-white/10 disabled:opacity-30 transition-colors flex items-center justify-center">←</button>
          <span className="text-xs text-white/40">{active + 1} / {tiers.length}</span>
          <button onClick={() => setActive((prev) => Math.min(tiers.length - 1, prev + 1))} disabled={active === tiers.length - 1} className="h-8 w-8 rounded-full border border-white/20 text-white/60 hover:bg-white/10 disabled:opacity-30 transition-colors flex items-center justify-center">→</button>
        </div>
      </div>
          ...
          {tiers.map((tier) => (
            <div key={tier.name} className="w-full shrink-0 px-1">
              <div className={`rounded-3xl border p-6 ${tier.color} relative`}>
                {tier.badge && <span className={`absolute top-4 right-4 rounded-full px-3 py-1 text-xs font-semibold ${tier.badgeColor}`}>{tier.badge}</span>}
                ...
                <Link href={tier.ctaHref} className={`mt-6 block w-full rounded-full py-2.5 text-center text-sm font-semibold transition-colors ${tier.ctaStyle}`}>
                  {tier.cta}
                </Link>
              </div>
            </div>
      ...
      <div className="flex justify-center gap-2">
        {tiers.map((_, i) => (
          <button key={i} onClick={() => setActive(i)} className={`h-1.5 rounded-full transition-all ${active === i ? 'w-6 bg-white' : 'w-1.5 bg-white/20'}`} />
        ))}
      </div>
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-10 sm:py-14">
        ...
        <header className="flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3 text-sm text-neutral-300">
            <Link className="text-sm text-white/70 hover:text-white transition-colors" href="/login">Sign in</Link>
            <Link className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-neutral-900 hover:bg-neutral-200 transition-colors" href="/plans">Get started free</Link>
          </nav>
        </header>
        ...
        <main className="mt-14 flex flex-col gap-16 pb-20">
          ...
          <section className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              <span className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-xs uppercase tracking-wide text-neutral-200">Flash SEC + multi-source intel</span>
              <h1 className="mt-6 text-balance text-4xl font-semibold leading-tight sm:text-5xl">Your fingertip on filings, fintech launches, and regulator moves.</h1>
              <p className="mt-5 text-base text-neutral-300">Signal Relay Hub watches AI labs, fintechs, and regulators in real time. When a filing or pricing change hits, we push it to Telegram, email, and your API stack within minutes.</p>
              <div className="mt-8 flex flex-wrap justify-center sm:justify-start gap-4">
                <Link href="/plans" className="rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-neutral-900 hover:bg-neutral-200 transition-colors">Start for free →</Link>
                <Link href="/feed" className="rounded-full border border-white/40 px-5 py-2.5 text-sm font-medium text-white/80 hover:bg-white/10 transition-colors">View live feed</Link>
              </div>
              <div className="mt-6 flex gap-4">
                <a href="https://t.me/signalrelayhub" target="_blank" rel="noreferrer" className="text-xs text-white/40 hover:text-white/70 transition-colors">📲 Join Telegram</a>
                <a href="https://discord.gg/4X9NGZuK" target="_blank" rel="noreferrer" className="text-xs text-white/40 hover:text-white/70 transition-colors">💬 Join Discord</a>
              </div>
            </div>
          ...
          <section>
            ...
            <div className="mt-4 flex flex-wrap gap-6 text-sm text-white/70">
              {logos.map((logo) => <span key={logo} className="font-medium tracking-tight">{logo}</span>)}
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/10">
                    ...
                    <th className="py-3 px-4 text-center"><div className="text-white font-semibold">Signal Relay Hub</div><div className="text-xs text-sky-300">£45/month</div></th>
                    <th className="py-3 px-4 text-center"><div className="text-white/70 font-medium">TradingView</div><div className="text-xs text-white/40">£15-60/month</div></th>
                    <th className="py-3 px-4 text-center"><div className="text-white/70 font-medium">Influencer Groups</div><div className="text-xs text-white/40">£3,000+</div></th>
                    <th className="py-3 px-4 text-center"><div className="text-white/70 font-medium">Trading Courses</div><div className="text-xs text-white/40">£1,000+</div></th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div className="mt-6 flex justify-center">
              <Link href="/plans" className="rounded-full bg-white px-6 py-2.5 text-sm font-semibold text-neutral-900 hover:bg-neutral-200 transition-colors">Get started free</Link>
            </div>
          </section>
